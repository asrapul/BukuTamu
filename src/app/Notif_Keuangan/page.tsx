...
function Page() {
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        const response = await axios.get("http://localhost:8000/api/notifications/type/keuangan_administrasis/unread");
        console.log(response.data);
        setNotifications(response.data);
        setUnreadCount(response.data.length); // Update unreadCount based on notifications
      } catch (err) {
  ...
  const markAsRead = async (id: string) => {
    try {
      ...
      setUnreadCount(prev => prev - 1); // Decrease unread count when marking as read
    } catch (err) {
  ...
  const markSelectedAsRead = async () => {
    try {
      ...
      setUnreadCount(prev => prev - selectedIds.length); // Decrease unread count by number of selected items
      setSelectedIds([]);
      setSelectAll(false);
    } catch (err) {
  ...
  return (
    <div className="relative min-h-screen p-8 flex items-center justify-center">
      ...
      <div className="relative bg-white border border-gray-300 rounded-lg p-6 shadow-lg w-full max-w-3xl backdrop-blur-md">
        ...
        <h2 className="text-xl font-semibold text-gray-700 text-center mb-4">Notifikasi Keuangan / Administrasi</h2>
        ...
        {loading ? (
          ...
          <p className="text-gray-500 text-center">Tidak ada notifikasi baru untuk Keuangan / Administrasi.</p>
        )}
      </div>
    </div>
  );
