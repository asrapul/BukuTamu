...
function Page() {
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        const response = await axios.get("http://localhost:8000/api/notifications/type/gurus/unread");
  ...
  return (
    <div className="relative min-h-screen p-8 flex items-center justify-center">
      ...
      <div className="relative bg-white border border-gray-300 rounded-lg p-6 shadow-lg w-full max-w-3xl backdrop-blur-md">
        ...
        <h2 className="text-xl font-semibold text-gray-700 text-center mb-4">Notifikasi Guru</h2>
        ...
        {loading ? (
          ...
          <p className="text-gray-500 text-center">Tidak ada notifikasi baru untuk Guru.</p>
        )}
      </div>
    </div>
  );
